...
# MCP mode only - no OpenAI function imports needed
...
# AlienRecon now uses MCP exclusively

# Load prompts from files with inline fallbacks
...
AGENT_SYSTEM_PROMPT = load_prompt_with_fallback(
    "mcp_system_prompt", _FALLBACK_SYSTEM_PROMPT
)

_FALLBACK_WELCOME_MESSAGE = """
...
# Load welcome message from file or use fallback
AGENT_WELCOME_MESSAGE = load_prompt_with_fallback(
    "welcome_message", _FALLBACK_WELCOME_MESSAGE
)

_FALLBACK_WELCOME_MESSAGE_WITH_TARGET = """
Hello! I'm Alien Recon, your AI assistant from Alien37.com, here to help you
...
# Load welcome message with target from file or use fallback
AGENT_WELCOME_MESSAGE_WITH_TARGET = load_prompt_with_fallback(
    "welcome_message_with_target", _FALLBACK_WELCOME_MESSAGE_WITH_TARGET
)

# MCP mode only - no OpenAI tools needed
...
def get_llm_response(client, history, system_prompt):
    ...
    # Handle None client in test mode
    if client is None:
        logging.warning("OpenAI client is None (test mode), returning None response")
        return None

    try:
        console.print("[yellow]Alien Recon is analyzing signals...[/yellow]", end="")

        # MCP mode - no tools parameter needed
