...
class PlanExecutor:
    ...
    def execute_next_step(self) -> Optional[dict[str, Any]]:
        ...
        try:
            logger.info(
                f"Executing step {current_step_idx}: {step.get('name', step['tool'])}"
            )
            ...
            result = self.tool_orchestrator.execute_tool(
                step["tool"], processed_args, use_cache=step.get("use_cache", True)
            )
    ...
    def _check_step_conditions(self, step: dict[str, Any]) -> bool:
        ...
        if "if_previous_success" in conditions:
            if not self.current_plan or self.current_plan["current_step"] == 0:
    ...
    def get_plan_results(self) -> dict[str, Any]:
        ...
        return dict(self.current_plan.get("results", {}))
