...
class CommandTool(ABC):
    ...
    def validate_input(self, **kwargs: Any) -> dict[str, Any]:
        ...
        if "wordlist" in kwargs:
            validated["wordlist"] = str(
                InputValidator.validate_wordlist_path(kwargs["wordlist"])
            )
    ...
    def execute(self, **kwargs: Any) -> ToolResult:
        ...
        try:
            ...
            if "scan_summary" not in parsed_results:
                parsed_results["scan_summary"] = (
                    parsed_results.get("error")
                    or f"{self.name.capitalize()} scan processing completed."
                )
