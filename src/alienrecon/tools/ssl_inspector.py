...
class SSLInspectorTool(CommandTool):
    ...
    def build_command(self, **kwargs: Any) -> list[str]:
        ...
        if not ip:
            raise ValueError(
                "IP address must be provided for SSL certificate inspection."
            )
        ...
        command_args = [
            ...
            f"{ip}:{port}",
            "-timeout",
            str(timeout),  # Use the timeout value
    ...
    def _parse_certificate_info(self, output: str) -> dict:
                    ...
                    if san_section:
                        ...
                        if ip_addresses:
                            if "subject_alt_names" not in cert_info:
                                cert_info["subject_alt_names"] = []
                            san_list = cert_info["subject_alt_names"]
        ...
        # Always try to parse SANs from raw output if we haven't found them yet
        if "subject_alt_names" not in cert_info:
            # Look for X509v3 Subject Alternative Name section in raw output
            ...
            if san_section_match:
                ...
                if ip_addresses:
                    ...
                    san_list = cert_info["subject_alt_names"]
                    if isinstance(san_list, list):
                        san_list.extend([f"IP:{ip.strip()}" for ip in ip_addresses])
