...
class FFUFTool(CommandTool):
    ...
    def build_command(self, **kwargs: Any) -> list[str]:
        ...
        filter_size = kwargs.get("filter_size", None)
        filter_words = kwargs.get("filter_words", None)
        filter_lines = kwargs.get("filter_lines", None)
        match_codes = kwargs.get("match_codes", None)
    ...
    def parse_output(
        self, stdout: str | None, stderr: str | None, mode: str = "vhost", **kwargs: Any
    ) -> ToolResult:
        """
        ...
        if stderr and stderr.strip():
            ...
            if any(
                ...
                # Store stderr diagnostics in findings instead of result
                if "diagnostics" not in result["findings"]:
                    result["findings"]["diagnostics"] = {}
                result["findings"]["diagnostics"]["stderr"] = stderr.strip()
        ...
        try:
            ...
            if first_brace != -1 and last_brace != -1 and last_brace > first_brace:
                ...
                try:
                    ...
                    # Store metadata in findings instead of result
                    if "metadata" not in result["findings"]:
                        result["findings"]["metadata"] = {}
                    if "commandline" in data:
                        result["findings"]["metadata"]["command_used"] = " ".join(
                            data["commandline"]
                        )  # ffuf provides this as a list
                    if "time" in data:
                        result["findings"]["metadata"]["scan_time"] = data["time"]
    ...
    def _extract_vhost_from_request(self, item: dict) -> str:
        ...
        if "Host:" in str(item):
            ...
            if host_match:
                return str(host_match.group(1))

        # Last resort: try to parse the URL if it contains one
        try:
            parsed = urlparse(item.get("url", ""))
            return str(parsed.path) if parsed.path else "unknown"
        except Exception:
            # Fallback to simple extraction
            if "/" in item.get("url", ""):
                return str(item.get("url", "").split("/")[-1])
            return "unknown"
