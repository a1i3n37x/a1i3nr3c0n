...
class NmapTool(CommandTool):
    ...
    def build_command(self, **kwargs: Any) -> list[str]:
        ...
        arguments = kwargs.get("arguments", "-sV -T4")
        """
    ...
    def parse_output(
        self, stdout: str | None, stderr: str | None, **kwargs: Any
    ) -> ToolResult:
        ...
        try:
            ...
            # Store scan arguments in findings instead of directly in result
            if "metadata" not in result["findings"]:
                result["findings"]["metadata"] = {}
            result["findings"]["metadata"]["scan_arguments_used"] = root.get(
                "args", args_context
            )
            for host_node in root.findall("host"):
                ...
                host_data: dict[str, Any] = {
                    ...
                    "status": cast(Element, host_node.find("status")).get(
                        "state", "unknown"
                    )
                    if host_node.find("status") is not None
                    else "unknown",
                    "open_ports": [],
                }
                ...
                if ports_node is not None:
                    for port_node in ports_node.findall("port"):
                        ...
                        if state_node is not None and state_node.get("state") == "open":
                            ...
                            cast(list[Any], host_data["open_ports"]).append(
                                {
                                    "port": port_info["port"],
                ...
                cast(list[Any], result["findings"]["hosts"]).append(host_data)
    ...
    def _parse_with_python_nmap(
        ...
        try:
            ...
            for host, host_data_dict in scan_results.items():
                ...
                cast(list[Any], results["hosts"]).append(host_info)

            if stderr:
                results["scan_summary"] = (
                    cast(str, results["scan_summary"])
