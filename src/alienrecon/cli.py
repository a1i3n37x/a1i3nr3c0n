...
# Import the refactored SessionController
from alienrecon.core.refactored_session_controller import (
    RefactoredSessionController as SessionController,
)
...
def main(
    ...
    """
    # Store dry_run flag in context for access by subcommands
    ctx.ensure_object(dict)
    ctx.obj["dry_run"] = dry_run

    if ctx.invoked_subcommand is None:
        # No subcommand provided - show help
        # User wants CLI commands, not automatic interactive mode
        ctx.get_help()
        raise typer.Exit()
...
def _init_ctf_environment(box_identifier: str) -> None:
...
def target(
    ...
    sc = (
        ...
        else SessionController(dry_run=dry_run)
    )
...
@app.command()
def status(ctx: typer.Context = typer.Option(None, hidden=True)) -> None:
    """
    ...
    cli_console.print(
        f"[dim]Chat history length: {len(sc.session_manager.chat_history)}[/dim]"
    )
    module_logger.info("`status` command executed.")


@app.command()
def save(ctx: typer.Context = typer.Option(None, hidden=True)) -> None:
    """
...
@app.command()
def load(ctx: typer.Context = typer.Option(None, hidden=True)) -> None:
    """
...
@app.command()
def clear(ctx: typer.Context = typer.Option(None, hidden=True)) -> None:
    """
...
def debrief(
    output: Optional[str] = typer.Option(
        ...
        "--output",
        "-o",
        help="Output file path for the report (default: prints to console)",
    ),
    format: str = typer.Option(
        "markdown",
        "--format",
        "-f",
        help="Report format: markdown, html (default: markdown)",
    ),
    ctx: typer.Context = typer.Option(None, hidden=True),
) -> None:
    """
    Generate a comprehensive debrief report from the current reconnaissance session.

    This command creates a detailed report including:
    - Executive summary of findings
    - Discovered services and ports
    - Web application findings
    - Potential vulnerabilities
    - Recommendations for next steps
    - Command history from the session
    """
    try:
        ...
        # Generate the report
        generator = ReportGenerator(sc.session_manager)
        report = generator.generate_debrief(output)

        # If no output file specified, print to console
        if not output:
            from rich.markdown import Markdown

            cli_console.print(Markdown(report))

        module_logger.info(f"`debrief` command executed. Output: {output}")

    except Exception as e:
        cli_console.print(f"[red]Error generating debrief report: {e}[/red]")
        module_logger.error(f"Error in debrief command: {e}", exc_info=True)
...
def recon(
    ...
    # Get dry_run from context if available
    dry_run = ctx.obj.get("dry_run", False) if ctx and ctx.obj else False

    module_logger.info(
        f"Recon command initiated with target: {target_address}, "
        f"novice_mode: {novice_mode}, dry_run: {dry_run}"
    )
    ...
    try:
        module_logger.info(f"Starting interactive reconnaissance for {sc.get_target()}")
        sc.start_interactive_session()  # This will contain the main loop
...
# agent_mode command removed - MCP is now the only mode
...
def nmap(
    ...
    try:
        ...
        if "nmap" not in sc.get_available_tools():
            cli_console.print(
                "[bold red]Nmap tool is not available (not installed or misconfigured).[/bold red]"
            )
            raise typer.Exit(code=1)
        result = sc.execute_tool(
            "nmap", {"target": target, "arguments": arguments}, show_result=False
        )

        # Handle dry_run mode output
        if result.get("dry_run") or result.get("status") == "dry_run":
            if "command" in result:
                cli_console.print(
                    "[yellow]DRY RUN: Command that would be executed:[/yellow]"
                )
                cli_console.print(f"[cyan]{result['command']}[/cyan]")
            return
...
def nikto(
    ...
    try:
        ...
        if "nikto" not in sc.get_available_tools():
            cli_console.print(
...
def hydra(
    ...
    try:
        ...
        if "hydra" not in sc.get_available_tools():
            cli_console.print(
...
def smb(
    ...
    try:
        ...
        if "smb" not in sc.get_available_tools():
            cli_console.print(
        ...
        result = sc.execute_tool(
            "smb",
            {"target": target, "enum_arguments": enum_arguments},
            show_result=False,
        )
        cli_console.print(
...
def http_fetch(
    ...
    try:
        ...
        sc = SessionController()
        # Check if http_fetcher is available
        if "http_fetcher" not in sc.get_available_tools():
            cli_console.print(
                "[bold red]HTTP fetcher tool is not available (not installed or misconfigured).[/bold red]"
            )
            raise typer.Exit(code=1)
        result = sc.execute_tool(
            "http_fetcher", {"url_to_fetch": url, "timeout": timeout}, show_result=False
        )
